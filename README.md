...
This compiles `sftp.spl`, `english.slg`, and `czech.slg` with statically linked C/C++ runtimes (no MinGW DLL dependencies). For deployment on other PCs, the 64-bit `libcrypto-3-x64.dll`, `libssh2.dll`, and `z.dll` libraries are included alongside `sftp.spl`.
