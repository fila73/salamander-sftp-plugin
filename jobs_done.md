...
| **Vylepšený výpočet velikosti složek (`Calc Size`)** | Přepracován výpočet velikosti na serveru: přidána rychlá server-side kalkulace `FastDirSize` přes SSH exec (`du -sb` / `du -sk` / `find`), interaktivní a nezamrzající dialog průběhu s možností okamžitého zrušení (**Cancel** přes `CDeleteProgressDlg`), ochrana proti zacyklení na symlincích a automatický update velikostí v panelu Salamandera. | `src/sftpconn.cpp`, `src/sftpconn.h`, `src/fs2.cpp` |
