...
*/
        ...
        if (SalamanderGeneral->TestFreeSpace(SalamanderGeneral->GetMsgBoxParent(),
                                             tmpExtractDir, totalSize, "Unpacking Sftp Archive"))
        {
            /*
      // demonstration of a progress dialog with a single progress bar
      salamander->OpenProgressDialog("Unpacking Sftp Archive", FALSE, NULL, FALSE);
...
*/
            ...
            salamander->OpenProgressDialog("Unpacking Sftp Archive", TRUE, NULL, FALSE);
                                           ...
                                           SalEnumSelection2 next, void* nextParam)
{
    ...
    salamander->OpenProgressDialog("Packing Sftp Archive", FALSE, NULL, FALSE);
    ...
    if (archivePath[0] == 0 ||
        SalamanderGeneral->TestFreeSpace(SalamanderGeneral->GetMsgBoxParent(),
                                         archivePath, totalSize, "Unpacking Sftp Archive"))
    {
                                               ...
                                               const char* archiveRoot, SalEnumSelection next, void* nextParam)
{
    ...
    salamander->OpenProgressDialog("Deleting Sftp Archive Entries", FALSE, NULL, FALSE);
    salamander->ProgressDialogAddText("reading directory tree...", FALSE);
...
CPluginInterfaceForArchiver::GetCacheInfo(char* tempPath, BOOL* ownDelete, BOOL* cacheCopies)
{
    ...
    if (tempPath[0] == 0 ||
        !SalamanderGeneral->SalPathAppend(tempPath, "Sftp Temporary Copies", MAX_PATH))
    {
...
void ClearTEMPIfNeeded(HWND parent)
{
    ...
    if (tmpDir[0] != 0 &&
        SalamanderGeneral->SalPathAppend(tmpDir, "Sftp Temporary Copies", 2 * MAX_PATH))
    {
        ...
        if (SalamanderGeneral->SalPathAppend(tmpDir, "SAL*.tmp", 2 * MAX_PATH))
        {
            ...
            if (tmpDirs.IsGood() && tmpDirs.Count > 0)
            {
                ...
                SalamanderGeneral->ExpandPluralString(buf2, 300,
                                                      ...
                                                      "by previous instances of Sftp plugin?",
                                                      1, &qwCount);
        ...
        TRACE_E("Sftp: Unable to clear TEMP directory: TEMP directory not defined!");
}
