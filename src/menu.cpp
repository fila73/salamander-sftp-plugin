...
void WINAPI SFTPOperationFromDisk(const char* sourcePath, SalEnumSelection2 next,
                                      void* nextParam, void* param)
{
    CSFTPOperFromDiskData* data = (CSFTPOperFromDiskData*)param;
...
*/
      ...
      CSFTPOperFromDiskData data;
      data.RetSize = &size;
      data.Parent = parent;
      data.Success = FALSE;
      SalamanderGeneral->CallPluginOperationFromDisk(PANEL_SOURCE, SFTPOperationFromDisk, &data);
...
*/
    ...
    case MENUCMD_ENTERDISKPATH: // example of validating a path entered by the user
    {
        ...
        {
            // the Sftpin does not perform anything here
        }
