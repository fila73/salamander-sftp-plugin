...
#define MENUCMD_CHECKSFTPTMPDIR 15 // first SFTP instance: optionally clean its temp dir with files unpacked by previous instances
...
struct CFSData
{
    char Rights[12]; // rwxr-xr-x
    char Owner[64];
    char Group[64];

    CFSData(const char* rights, const char* owner, const char* group)
    {
        lstrcpynA(Rights, rights != NULL ? rights : "", sizeof(Rights));
        lstrcpynA(Owner, owner != NULL ? owner : "", sizeof(Owner));
        lstrcpynA(Group, group != NULL ? group : "", sizeof(Group));
    }
    ~CFSData() {}
